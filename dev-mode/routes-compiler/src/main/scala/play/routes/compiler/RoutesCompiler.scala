...
object RoutesCompiler {
    ...
    def unapply(file: File): Option[GeneratedSource] = {
      ...
      if (lines.contains("// @GENERATOR:play-routes-compiler")) {
        Some(new GeneratedSource {
          val source: Option[File] = lines.find(_.startsWith(SourcePrefix)).map { line =>
            val relativePath = line.trim.drop(SourcePrefix.length)
            val resolvedPath = file.toPath.getParent.resolve(relativePath)
  ...
  def compile(
    ...
    RoutesFileParser.parse(routeFile).right.map { rules =>
      val generated = generator.generate(task, generatedDir, namespace, rules)
      generated.map {
